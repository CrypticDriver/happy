...
export const agentKeys = ['claude', 'codex', 'gemini', 'openclaw', 'kiro'] as const;
...
export const AgentDefaultOverridesSchema = z.object({
    ...
    kiro: AgentDefaultOverrideSchema.optional(),
}).passthrough().default({});
...
const codeAgentDefaults: Record<AgentKey, AgentDefaultConfig> = {
    ...
    codex: { permissionMode: 'yolo', modelMode: 'openai.gpt-5.6-sol', effortLevel: 'medium' },
    gemini: { permissionMode: 'default', modelMode: 'gemini-2.5-pro', effortLevel: null },
    openclaw: { permissionMode: 'default', modelMode: 'default', effortLevel: null },
    kiro: { permissionMode: 'default', modelMode: 'default', effortLevel: null },
};

export function normalizeAgentKey(flavor: string | null | undefined): AgentKey {
    if (flavor === 'codex' || flavor === 'gemini' || flavor === 'openclaw' || flavor === 'kiro') {
        return flavor;
    }
    return 'claude';
}
